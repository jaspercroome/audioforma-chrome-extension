const draw = ({ keyOctaveAmplitudes, videoElement }) =>{
    
  const { clientWidth: width, clientHeight: height, offsetTop: top, offsetLeft: left } = videoElement;
...
amplitudeScale.clamp();
  ...
  const getCoords = (noteOctave)=>{
    const note = noteOctave.includes('#') ? noteOctave.slice(0, 2) : noteOctave.slice(0, 1);
  ...
  if (keyOctaveAmplitudes) {
    const amplitudesSorted = Object.entries(keyOctaveAmplitudes).sort((a, b) => b[1] - a[1]);
    ...
    const [x,y,degrees] = getCoords(strongestNote[0]??'')
      ...
      const lineGenerator = d3
        ...
        .x(d => d[0])
        .y(d => d[1])
        .curve(strongestNote[1] > buffer *.75 ? d3.curveLinearClosed : d3.curveBasisClosed);
    ...
    function renderPath() {
      ...
      .attrTween('d', function() {
        return d3.interpolate(path.getAttribute('d') || '', lineGenerator(pathData));
      })
        .attrTween('fill',()=>{
          return d3.interpolate(path.getAttribute('fill')||'#ff5200',color)
        })
        .attrTween('stroke',()=>{
          return d3.interpolate(path.getAttribute('fill')||'#ff5200',color)
        })
...
// Update points
      ...
      octaves.forEach((octave)=>{
        ...
        const translateValue = `${width / 2}, ${
          getYMove(adjustedHeight, octaveIndex)
        }`;
        ...
        noteNameValues.forEach((note,index)=>{
          ...
          if (probablyPercussion){
            ...
            rect.setAttribute('width',Math.min(amplitudeScale(amplitude) * 2, 100))
